...
module.exports = function(filenames,done){
  ...
  function renderPosts(callback){
    ...
    async.eachLimit(orderedNames,ASYNC_LIMIT,(name,cb)=>{
      fs.readFile(path.join(process.cwd(),'_bumblersrc/hydrated-posts',name),'utf8',(er,file)=>{
        if (er){return cb(er);}
  ...
  function renderPages(callback){
    ...
    async.eachOfLimit(postGroups,ASYNC_LIMIT,(postNames,index,cb0)=>{
      ...
      async.eachSeries(postNames,(name,cb)=>{
        fs.readFile(path.join(process.cwd(),'_bumblersrc/hydrated-posts',name),'utf8',(er,post)=>{
          if (er){return cb(er)}
          try{
  ...
  function renderAtomFeed(cb){
    ...
    async.eachOfSeries(first10Names,(name,index,done)=>{
      fs.readFile(path.join(process.cwd(),'_bumblersrc/hydrated-posts',name),'utf8',(er,contents)=>{
        if(er){
          return done(er);
        }
        try{
          posts[index] = JSON.parse(contents);
        }catch(e){
          return done(er);
        }
        return done(null);
