...
module.exports = function(filenames,done){
  ...
  var addDefaults = function(locals){
    ...
    return _.assign({},getTemplateDefaults(),locals);
  }
  ...
  function getLinks(pageNumber,tag,count){
    ...
    if (tag){
      var idxPage = linkTo('tag',tag);
      ret.firstPage = idxPage;

      ret.nextPage = (pageNumber && pageNumber < count) ? linkTo('tag',tag,pageNumber+1) : null;
      if (pageNumber == 2){
        ret.previousPage = ret.firstPage;
      }else{
        ret.previousPage = (pageNumber && pageNumber > 1) ? linkTo('tag',tag,pageNumber-1) :null;
      }
      ret.lastPage = (count > 1) ? linkTo('tag', tag, count) :idxPage;

      if (settings.reverseOrder){
        // flip first and last
        ret.lastPage = idxPage;
        ret.firstPage = linkTo('tag',tag,1)
      }
    }

    else{
      var idxPage = linkTo('root');
      ret.firstPage = idxPage;

      ret.nextPage = (pageNumber && pageNumber < count) ? linkTo('page',pageNumber+1) : null;
      if (pageNumber == 2){
        ret.previousPage = ret.firstPage;
      }
      else {
        ret.previousPage = (pageNumber && pageNumber > 1) ?linkTo('page',pageNumber-1) :null
      }
      ret.lastPage = (count > 1) ?linkTo('page',count) :idxPage;

      if (settings.reverseOrder){
        // flip first and last
        ret.lastPage = idxPage;
        ret.firstPage = linkTo('page',1);
      }
  ...
  function renderPosts(callback){
    ...
    async.eachLimit(orderedNames,ASYNC_LIMIT,(name,cb)=>{
      fs.readFile(path.join(process.cwd(),'_bumblersrc/posts',name),'utf8',(er,file)=>{
        ...
        var filename = path.join(process.cwd(),post.permalink);
    ...
    async.eachOfLimit(postGroups,ASYNC_LIMIT,(postNames,index,cb0)=>{
      ...
      async.eachSeries(postNames,(name,cb)=>{
        fs.readFile(path.join(process.cwd(),'_bumblersrc/posts',name),'utf8',(er,post)=>{
          if (er){return cb(er)}
          try{
            posts.push(hydrate(mdToJson(post)));
          }catch(e){
            return cb(e);
          }
          return cb(null);
          ...
          var locals = {
            page:{
              ...
              url:urlForHref(linkTo('page',pageN))
            }
          };

          var toWrite = [ [ path.join(process.cwd(),linkTo('page',pageN)), locals ] ]
          ...
          if (pageN == 1 && !settings.reverseOrder || pageN == siteInfo.pageCount  && settings.reverseOrder){
            var newLocals = {
              page:{
                ...
                links:getLinks(pageN),
                isIndex:true,
                url:urlForHref(linkTo('root'))
              }
            };
            toWrite.push([ path.join(process.cwd(),linkTo('root')) , newLocals ])
          }
      ...
      async.eachOfLimit(postGroups,ASYNC_LIMIT,(postNames,index,cb0)=>{
        ...
        async.eachSeries(postNames,(name,cb)=>{
          fs.readFile(path.join(process.cwd(),'_bumblersrc/posts',name),'utf8',(er,post)=>{
            ...
            if (er){return cb0(er)}
            var locals ={
              site:_.defaults({pageCount:nP},siteInfo),
              page:{
                number:pageN,
                posts:posts||[],
                links:getLinks(pageN,tag,postGroups.length),
                tag:tag,
                url:urlForHref( linkTo('tag',tag,pageN) )
              }
            };
            // console.log('built page'+pageN+', writing...');

            var filenames = [path.join(process.cwd(),linkTo('tag',tag,pageN))];

            if (pageN == 1 && !settings.reverseOrder || pageN == nP  && settings.reverseOrder){
              filenames.push( path.join(process.cwd(),linkTo('tag',tag)) )
            }
      ...
      async.eachOfLimit(postGroups,ASYNC_LIMIT,(postNames,index,cb0)=>{
        ...
        async.eachSeries(postNames,(name,cb)=>{
          fs.readFile(path.join(process.cwd(),'_bumblersrc/posts',name),'utf8',(er,post)=>{
            ...
            var feedPageName = isIndex?linkTo('feed'):linkTo('feed',pageN);

            var toWrite = [ [ path.join(process.cwd(),feedPageName), locals ] ]
