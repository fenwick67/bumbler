...
const jsonToMd = require('./jsonToMd')
...
procedures.getPostIds = function(done){
  glob(path.join(process.cwd(),'_bumblersrc/posts/*.md'),function(er,results){
...
procedures.putPost = function(post,done){
  var p = new Post(post);
  var filename = path.join(process.cwd(),'_bumblersrc/posts',p.id+'.md');
  fs.outputFile(filename,jsonToMd(p),done);
}

procedures.getPost = function(id,done){
  var filename = path.join(process.cwd(),'_bumblersrc/posts',id+'.md');
...
procedures.getCustomPages = function(done){
  glob(path.join(process.cwd(),'_bumblersrc/pages/*.md'),(er,files)=>{
    if (er){
      return done(er);
    }
...
procedures.getCustomPage = function(name,done){
  const filename = path.join(process.cwd(),'_bumblersrc/pages/'+name+'.md');
  fs.readFile(filename,'utf8',function(er,text){
    if(er){return done(er)}
    try{
      var j = mdToJson(text);
...
procedures.putCustomPage = function(page,done){
  var filename = path.join(process.cwd(),'_bumblersrc/pages',page.id+'.md');
  fs.outputFile(filename,jsonToMd(page),done);
}
