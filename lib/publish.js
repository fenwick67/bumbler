...
exports.commit = function commit(cb){
  var add = spawn('git',['add','-A'],{cwd:cwd()})
  logProcess(add);
  ...
  add.on('close',code=>{
    ...
    var commit = spawn('git',['commit','-m','commit from Bumbler'],{cwd:cwd()})
    logProcess(commit);
...
exports.push = function push(branch,cb){
  ...
  var push = spawn('git',['push','bumbler',branch],{cwd:cwd()});
  logProcess(push);
...
exports.addRemote = function addRemote(url,cb){
  var remote = spawn('git',['remote','add','bumbler',url],{cwd:cwd()});
  logProcess(remote);
...
exports.setRemote = function setRemote(url,cb){
  var remote = spawn('git',['remote','set-url','bumbler',url],{cwd:cwd()});
  logProcess(remote);
  remote.on('close',code=>{
    if (code){
      return cb(new Error("'git remote add' exited nonzero status"))
...
exports.init = function init(cb){
  var init = spawn('git',['init'],{cwd:cwd()});
  logProcess(init);
  init.on('close',code=>{
    if (code){
...
exports.check = function check(cb){
  var status = spawn('git',['status'],{cwd:cwd()});
  logProcess(status);
